...
ifneq ($(abspath $(ROOT_DIR)),$(shell go env GOPATH)/src/github.com/PatrickLaabs/cluster-api-addon-provider-cdk8s) # Changed to cdk8s
...
release-notes: $(RELEASE_NOTES_DIR) $(RELEASE_NOTES)
	...
	echo ":rotating_light: This is a RELEASE CANDIDATE. Use it only for testing purposes. If you find any bugs, file an [issue](https://github.com/kubernetes-sigs/cluster-api-addon-provider-cdk8s/issues/new)." > $(RELEASE_NOTES_DIR)/release-notes-$(RELEASE_TAG).md; \
	else \
...
promote-images: $(KPROMO)
	$(KPROMO) pr --project cluster-api-cdk8s --tag $(RELEASE_TAG) --reviewers "$(IMAGE_REVIEWERS)" --fork $(USER_FORK) --image cluster-api-cdk8s-controller
...
$(CONVERSION_VERIFIER): $(TOOLS_DIR)/go.mod # Build conversion-verifier from tools folder.
	cd $(TOOLS_DIR); go build -tags=tools -o $(BIN_DIR)/$(CONVERSION_VERIFIER_BIN) github.com/PatrickLaabs/cluster-api-addon-provider-cdk8s/hack/tools/conversion-verifier # Changed to cdk8s
...
$(RUNTIME_OPENAPI_GEN): $(TOOLS_DIR)/go.mod # Build openapi-gen from tools folder.
	cd $(TOOLS_DIR); go build -tags=tools -o $(BIN_DIR)/$(RUNTIME_OPENAPI_GEN_BIN) github.com/PatrickLaabs/cluster-api-addon-provider-cdk8s/hack/tools/runtime-openapi-gen # Changed to cdk8s
...
$(TILT_PREPARE): $(TOOLS_DIR)/go.mod # Build tilt-prepare from tools folder.
	cd $(TOOLS_DIR); go build -tags=tools -o $(BIN_DIR)/tilt-prepare github.com/PatrickLaabs/cluster-api-addon-provider-cdk8s/hack/tools/tilt-prepare # Changed to cdk8s
